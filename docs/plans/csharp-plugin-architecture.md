...
4. World and block parity. The first landed slice generates `Cube.Pos`, `Cube.Range`,
   `Cube.Face`, `World.Block`, `World.SetOpts`, `World.Tx.Block`, `World.Tx.SetBlock`, 79
   stateless block types, and `Block.Sand`. `Player.Context : World.Context : World.Tx`, so event
   handlers use world operations directly just like embedded Dragonfly contexts. Registered block
   states supply canonical private codecs; public plugins never handle identifiers, NBT, world
   handles, or host errors. Remaining stateful blocks, liquids, structures, world methods, custom
   blocks, and block models land incrementally.
...
`World.SetOpts` flags. NativeAOT and host-call tests verify the public shape and transaction-safe
transport.
