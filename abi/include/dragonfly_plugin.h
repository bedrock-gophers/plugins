...
typedef DfStatus (*DfInventoryMenuSubmitFn)(void *callback_context, DfInvocationId invocation, const DfPlayerSnapshot *submitter, const DfItemStackViewV3 *item);
...
/*
 * A structurally valid inventory menu transfers callback_context ownership to
 * the host, even when player_inventory_menu_send returns an error. submit may
 * run zero or more times. Exactly one of close or drop terminates ownership.
 */
typedef struct {
    DfStringView name;
    uint32_t container;
    ...
    DfInventoryMenuSubmitFn submit;
...
typedef DfStatus (*DfHostPlayerInventoryMenuSendFn)(uint64_t context, DfInvocationId invocation, DfPlayerId player, uint8_t update, const DfInventoryMenuView *menu);
